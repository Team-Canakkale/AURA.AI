...
import { BloodDataSchema, generateDietPlan } from '../services/gemini';
...
export const parseBloodPdf = async (req: Request, res: Response) => {
    try {
        ...
        const metrics = extractBloodMetrics(parsed.text || '');
        const validated = BloodDataSchema.parse(metrics);
...
export const generateDietPlanHandler = async (req: Request, res: Response) => {
    try {
        const validated = BloodDataSchema.parse(req.body);
